...
(For this section, Explorer++Portable is the executable base file name, normally
the App ID of the package.)

Some configuration in the PortableApps.com Launcher can be overridden by the
user in an INI file next to Explorer++Portable.exe called Explorer++Portable.ini.  If
...
PortableApps.comLauncher.ini from this directory to Explorer++Portable.ini next to
Explorer++Portable.exe. The options in the INI file are as follows::
